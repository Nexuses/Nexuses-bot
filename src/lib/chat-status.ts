export function openingStatus(message: string, fileNames: string[]) {
  ...
  if (/public link|share link|share (this |the )?html|share (page|email)/i.test(message)) {
    return "Creating a public link…";
  }
  if (/keep (updating|syncing)|continue (updating|syncing)|auto[- ]?(update|sync)|until .{0,40}(complete|done|end)/i.test(message)) {
...
export function statusForTool(name: string) {
  const labels: Record<string, string> = {
    ...
    share_html_begin: "Starting chunked HTML publish…",
